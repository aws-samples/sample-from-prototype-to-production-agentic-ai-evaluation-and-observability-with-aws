"""
Custom Evaluators for E-Commerce Product Catalog Agent (Production Batch Evaluation)
...
Mirrors the evaluators in Module 02 for consistency between offline and production evaluation.
"""
...
- 0.3: Poor - wrong tool selected
- 0.0: Incorrect - completely wrong tool or no tool when one was needed
...
def get_all_custom_evaluators():
    """Get all custom evaluators as a dictionary (keyed by name) for batch evaluation."""
    return {
...
def get_evaluator_list():
    """Get evaluators as a list (for compatibility with Experiment API)."""
    return [
