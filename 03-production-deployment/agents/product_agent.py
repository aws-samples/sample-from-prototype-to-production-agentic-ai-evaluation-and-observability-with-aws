"""
Product Agent - AgentCore Runtime Agent

This agent handles product-related queries using MCP tools from AgentCore Gateway.
...
# FastAPI app
app = FastAPI(title="Product Agent Server", version="1.0.0")
...
def initialize_agent():
    """Initialize the Product Agent with MCP tools from Gateway."""
    global mcp_client, product_tools, product_agent
    ...
    try:
        ...
        system_prompt = """You are the Product Agent, a specialized e-commerce assistant focused on product catalog.

Your capabilities include:
- Searching products in the catalog (use ProductTools___search_products)
- Getting detailed product information (use ProductTools___get_product_details)
- Checking inventory availability (use ProductTools___check_inventory)
- Providing product recommendations (use ProductTools___get_product_recommendations)
- Comparing multiple products (use ProductTools___compare_products)
- Explaining return policies (use ProductTools___get_return_policy)

Always use the appropriate tools to get accurate, real-time information from the database.
Be helpful and provide relevant product details and recommendations."""
...
async def invoke_agent(request: InvocationRequest):
    ...
    try:
        ...
        result = product_agent(user_message)

        response = {
            "message": str(result),
            "agent": "product-agent",
            "tools_used": len(product_tools),
