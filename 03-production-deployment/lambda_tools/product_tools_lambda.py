...
def search_products(query: str, category: str = None, max_results: int = 5) -> dict:
    ...
    try:
        ...
        return {'success': False, 'error': str(e), 'message': f'Error searching for products: {query}'}
...
def get_product_details(product_id: str) -> dict:
    ...
    try:
        ...
        if 'Item' not in response:
            return {'success': False, 'product_id': product_id, 'message': f'Product {product_id} not found.'}
        ...
        return {'success': False, 'error': str(e), 'message': f'Error getting product details for {product_id}'}
...
def check_inventory(product_id: str) -> dict:
    ...
    try:
        ...
        if 'Item' not in response:
            return {'success': False, 'product_id': product_id, 'message': f'Product {product_id} not found in inventory system.'}
        ...
        return {'success': False, 'error': str(e), 'message': f'Error checking inventory for {product_id}'}
...
def get_product_recommendations(category: str = None, price_max: float = None, limit: int = 5) -> dict:
    ...
    try:
        ...
        if not items:
            return {'success': True, 'recommendations': [], 'message': 'No products match your criteria.'}
        ...
        return {'success': True, 'recommendation_count': len(recommendations), 'recommendations': recommendations}
    except Exception as e:
        return {'success': False, 'error': str(e), 'message': 'Error getting product recommendations'}
...
def compare_products(product_ids: list) -> dict:
    ...
    try:
        if len(product_ids) < 2:
            return {'success': False, 'message': 'Please provide at least 2 product IDs to compare.'}
        ...
        if len(products) < 2:
            return {'success': False, 'message': 'Could not find enough products to compare.'}

        return {'success': True, 'comparison_count': len(products), 'products': products}
    except Exception as e:
        return {'success': False, 'error': str(e), 'message': 'Error comparing products'}
...
def get_return_policy(product_id: str = None) -> dict:
    ...
    try:
        ...
        return {'success': False, 'error': str(e), 'message': 'Error getting return policy'}
...
def create_product(product_id: str, name: str, category: str, price: float,
                   ...
                   return_policy: str = "30-day return policy") -> dict:
    ...
    try:
        ...
        try:
            ...
            return {'success': False, 'error': 'Invalid specifications format. Please provide valid JSON.'}
        ...
        return {
            'success': True, 'product_id': product_id, 'name': name,
            'category': category, 'price': price, 'stock_quantity': stock_quantity,
            'message': f'Product {product_id} ({name}) created successfully.'
        }
    except Exception as e:
        return {'success': False, 'error': str(e), 'message': f'Error creating product {product_id}'}
...
def update_product(product_id: str, updates: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            return {'success': False, 'error': 'Invalid updates format. Please provide valid JSON.'}
        ...
        return {'success': False, 'error': str(e), 'message': f'Error updating product {product_id}'}
...
def delete_product(product_id: str) -> dict:
    ...
    try:
        ...
        return {
            'success': True, 'product_id': product_id, 'name': product_name,
            'action': 'soft_delete',
            'message': f'Product {product_id} ({product_name}) has been discontinued and removed from active catalog.'
        }
    except Exception as e:
        return {'success': False, 'error': str(e), 'message': f'Error deleting product {product_id}'}
...
def update_inventory(product_id: str, new_quantity: int, restock_date: str = None) -> dict:
    ...
    try:
        ...
        result = {
            ...
            'new_quantity': new_quantity, 'in_stock': in_stock,
        ...
        return {'success': False, 'error': str(e), 'message': f'Error updating inventory for {product_id}'}


def update_pricing(product_id: str, new_price: float,
                   sale_price: float = None, sale_end_date: str = None) -> dict:
    ...
    try:
        ...
        if sale_price is not None and sale_price >= new_price:
            return {'success': False, 'error': f'Sale price (${sale_price}) must be less than regular price (${new_price}).'}
        ...
        if sale_price is not None:
            if not sale_end_date:
                return {'success': False, 'error': 'sale_end_date is required when setting a sale_price.'}
        ...
        return {'success': False, 'error': str(e), 'message': f'Error updating pricing for {product_id}'}
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not tool_name:
            return {'statusCode': 400, 'body': json.dumps({'error': 'No tool name specified'})}
